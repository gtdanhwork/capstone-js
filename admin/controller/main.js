...
function errorsShow(e) {
	...
	Object.entries(e).forEach(([k, v]) => {
		const el = $('error' + k.charAt(0).toUpperCase() + k.slice(1));
...
function stats() {
	...
	$('samsungCount').textContent = products.filter(
		(x) => String(x.type).toLowerCase() === 'samsung',
	).length;
	$('iphoneCount').textContent = products.filter(
...
function render(list = products) {
	if (!list.length) {
		tbody.innerHTML =
			'<tr><td colspan="7" class="text-center py-5">Không có sản phẩm.</td></tr>';
		stats();
		return;
	}
...
function apply() {
	...
	if ($('sortPrice').value === 'asc') a.sort((x, y) => x.price - y.price);
...
function reset() {
	...
	$('btnSaveProduct').textContent = 'Thêm sản phẩm';
}
...
async function load() {
	try {
		...
		console.error(e);
		tbody.innerHTML =
			'<tr><td colspan="7"><div class="alert alert-danger m-3">Không thể kết nối API.</div></td></tr>';
	}
}
...
form.addEventListener('submit', async (e) => {
	e.preventDefault();
	const d = data(),
		er = validation.validateProduct(d);
	...
	try {
		...
		console.error(err);
		toast('Thao tác API thất bại.', 'danger');
	}
});

tbody.addEventListener('click', async (e) => {
	const ed = e.target.closest('.btnEdit'),
		del = e.target.closest('.btnDelete');
	if (ed) {
		const p = products.find((x) => String(x.id) === String(ed.dataset.id));
		if (!p) return;
		editingId = p.id;
		$('modalTitle').textContent = 'Cập nhật sản phẩm';
		$('btnSaveProduct').textContent = 'Lưu thay đổi';
		fill(p);
		errorsClear();
	...
	if (del) {
		...
		try {
			...
			toast('Xóa sản phẩm thất bại.', 'danger');
		}
	}
});

$('btnOpenAdd').addEventListener('click', reset);
$('searchName').addEventListener('input', apply);
$('sortPrice').addEventListener('change', apply);
